...
. Expand the Maven module `contacts.client` in the Eclipse package explorer
...
. Expand the Maven module `contacts.shared` in the Eclipse package explorer
...
For security reasons, the browser is not allowed to load any content from other servers by default. For our demo images, we add a well-considered exception. Open the _config.properties_ file of your UI server project and make sure it contains the following line:
